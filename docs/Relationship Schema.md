...
| `agency` | String | Required | Certification agency (PADI, NAUI, SSI) |
...
// Availability searches by date and status
...
// Financial reporting
db.payments.createIndex({ "status": 1, "completed_at": -1 })
```

### Text Indexes
```javascript
// Search functionality
db.activities.createIndex({ "name": "text", "description": "text" })
db.resorts.createIndex({ "name": "text", "location": "text" })
db.courses.createIndex({ "name": "text", "agency": "text" })
```
...
available\_rooms → Integer



status → ENUM('available', 'fully\_booked')



images → JSON Array of URLs/Strings (Multivalued)



space per room → JSON Array of URLs/Strings (Multivalued)
...
\## ActivityBookings (Junction Table)



booking\_id (PK) → String/ObjectId
...
activity\_id (FK → Activities)



seats\_booked → Integer
...
booking\_status → ENUM('pending', 'confirmed', 'cancelled', 'completed')



CONSTRAINT: seats\_booked ≤ Activities.total\_seats



\## VehicleBookings (Junction Table)
...
vehicle\_id (FK → Vehicles)



pickup\_date → DateTime
...
CONSTRAINT: seats\_booked ≤ Vehicles.seat\_capacity



\## ResortBookings (Junction Table)
...
resort\_id (FK → Resorts)



check\_in\_date → DateTime



check\_out\_date → DateTime



rooms\_booked → Integer
...
CONSTRAINT: rooms\_booked ≤ Resorts.available\_rooms



\## CourseBookings (Junction Table)
...
payment\_id (PK) → String/ObjectId



booking\_id (FK → Composite: references specific booking type)



booking\_type → ENUM('activity', 'vehicle', 'resort', 'course')
...
method → ENUM('card', 'bankTransfer', 'cash')
...
invoice\_id (PK) → String/ObjectId



payment\_id (FK → Payments)



invoice\_number → String, Unique



issued\_date → DateTime
...
CONSTRAINT: total\_amount = Payments.total\_amount + tax\_amount
...
CONSTRAINT: amount ≤ Payments.total\_amount - SUM(existing refunds for this payment)
...
contact\_id (PK) → String/ObjectId
